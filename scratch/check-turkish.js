...
async function checkTurkish() {
    const { data: flights } = await supabase
        .from('flights')
        .select('*')
        .eq('airline', 'Turkish Airlines')
        .or('departure_city.ilike.%bag%,arrival_city.ilike.%bag%');

    console.log("Turkish Airlines flights with 'bag':");
    flights.forEach(f => {
        console.log(`ID: ${f.id} | ${f.departure_city} -> ${f.arrival_city} | Date: ${f.departure_date}`);
    });
}
